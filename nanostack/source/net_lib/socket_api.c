...
int8_t socket_open(uint8_t protocol, uint16_t identifier, void (*passed_fptr)(void *))
{
    ...
    if(protocol == SOCKET_TCP){
        tr_debug("TCP Socket open in progress");
    }
    switch (protocol) {
        case SOCKET_ICMP:
        ...
        case SOCKET_TCP:
            ...
            tr_debug("TCP Socket open in progress 2");
            break;
#endif /* NO_TCP */
        case SOCKET_RAW:
            if (identifier > 0xff) {
    ...
    tr_debug("Code : %d", protocol);
    if(protocol == IPV6_NH_TCP){
        tr_debug("TCP Socket before create");
    }
    switch (socket_create(family, type, protocol, &sid, identifier, passed_fptr, false)) {
        case eOK:
            if(protocol == IPV6_NH_TCP){
                tr_debug("TCP Socket eOK");
            }
            return sid;
        case eBUSY:
            if(protocol == IPV6_NH_TCP){
                tr_debug("TCP Socket eBUSY");
            }
            return -2;
        case eFALSE:
            if(protocol == IPV6_NH_TCP){
                tr_debug("TCP Socket eFalse");
            }
        default:
            if(protocol == IPV6_NH_TCP){
                tr_debug("TCP Socket eOther");
            }
            break;
    }
...
int8_t socket_connect(int8_t socket, ns_address_t *address, uint8_t randomly_take_src_number)
{
    ...
    if (inet_pcb->local_port == 0) {
        ...
        tr_debug("Random port selected : %d", inet_pcb->local_port);
        if (inet_pcb->local_port == 0) {
            return -7;
        }
    }
