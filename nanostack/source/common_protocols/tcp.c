                                 ...
                                 const uint8_t remote_ip[static 16], uint16_t remote_port)
{
    ...
    if (!secret.initialised) {
        /* rand should do - we're not after total cryptographic security, and
         ...
         */
        rand_get_n_bytes_random(secret.key, sizeof secret.key);
...
tcp_session_t *tcp_session_ptr_allocate(inet_pcb_t *inet_pcb, tcp_session_t *from_time_wait)
{
    ...
    cur = malloc(sizeof(tcp_session_t));
    if (cur) {
        ...
        cur->send_window = 0;
        if (from_time_wait) {
            cur->send_next = from_time_wait->send_next + UINT32_C(250000) + rand_get_16bit();
...
tcp_error tcp_session_send(tcp_session_t *tcp_info, buffer_t *buf)
{
    ...
    if (!tcp_info->busy) {
        tr_debug("TCP socket not busy");
        tcp_info->timer = 0;
        tcp_info->retry = 0;
        tcp_info->busy = true;
        tcp_segment_start(tcp_info, false);
    }else{
        tr_debug("TCP socket busy");
    }
...
void tcp_session_data_received(tcp_session_t *tcp_info)
{
    ...
    if (tcp_compute_window_incr(tcp_info)) {
        ...
        if((buf->options.code & TCP_FLAG_ACK) && !(buf->options.code & TCP_FLAG_SYN)){
            tr_debug("This is an ACK message received");
        }
        tcp_build(buf, tcp_info);
    }
}
